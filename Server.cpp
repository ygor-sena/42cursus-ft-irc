...
/*   Updated: 2024/04/28 16:07:56 by gilmar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::_accept_new_client()
{
	...
	int incofd = accept(_server_fdsocket, (sockaddr *)&(cli_add), &len);
	if (incofd == -1)
	...
	new_poll.revents = 0;
	cli.SetFd(incofd);
	cli.setIpAdd(inet_ntoa((cli_add.sin_addr)));
...
//-> This fuction is TMP for clean the clientes
