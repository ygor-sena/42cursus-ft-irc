...
void Server::_handler_client_mode(const std::string &buffer, const int fd) {
    ...
    if (!_parse_mode_command(modes, channel, _get_client(arg), arg, fd)) {
        _send_response(fd, ERR_INVALIDMODEPARM(admin->get_nickname(), modes[1]));
        _reply_code = 696;
        return;
    }
}

bool Server::_parse_mode_command(const std::string &modes, Channel* channel, Client* client, const std::string &arg, const int fd) {
    bool set = false;
    char mode = 0;
...
bool Server::_handler_mode_flags(Channel* channel, Client* client, char mode, bool set, const std::string &arg, const int fd) {
    switch (mode) {
        case 'i':
...
void Server::_handle_password_mode(Channel* channel, const std::string &argument, bool set) {
    if (set) {
...
void Server::_handle_operator_privileges_mode(Channel* channel, Client* client, bool set) {
    if (set) {
        if (client) {
...
void Server::_handle_limit_mode(Channel* channel, Client* client, const std::string &argument, bool set, const int fd) {
    if (set) {
        if (!argument.empty()) {
