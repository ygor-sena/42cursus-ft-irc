...
/*   Updated: 2024/06/01 11:55:22 by caalbert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 *
 * This array stores the quotes used by the MarvinBot class. Each quote is
 * represented as a C-style string. The quotes are accessed using the predefined
 * constants in the MarvinBot class.
 */
 ...
 * from a client. It checks if the client is logged in and if they are already
 * in any channel. Depending on the client's state, it sends the appropriate
 * response back to the client.
 *
 * @param buffer The command buffer received from the client (not used in this
 * function).
 ...
 *
 * This function is responsible for handling the bot time command received from
 * a client. It checks if the client is logged in and if they are in any
 * channel. If the client is not registered, it sends an error response. If the
 * client is registered and in a channel, it sends the current time to the
 * client.
 *
 ...
 * This function is responsible for processing the "bot_whoami" command received
 * from a client. It checks if the client is logged in and sends the appropriate
 * response based on the client's status.
 *
 ...
 *
 * This function is responsible for processing the BOT_WHOIS command received
 * from the server. It extracts the nickname from the buffer and retrieves the
 * corresponding client object. If the client is not logged in, it sends an
 * error response. If the client is logged in and is in a channel, it sends a
 * response with the client's information. If the client is logged in but not in
 * a channel, it sends a response indicating that the client is not in a
 * channel. If the client does not exist, it sends a response indicating that
 ...
 *
 * This function is responsible for handling the bot quote command received from
 * a client. It checks if the client is logged in and sends an appropriate
 * response based on the client's status.
 *
 * @param buffer The command buffer received from the client.
